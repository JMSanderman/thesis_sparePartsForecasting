...
def cross_validation_score_croston(train, cv,alpha_value):
    '''
...
#######################################################################################
#                 4. Apply Croston's model to the spare parts                         #
#######################################################################################

# create list for alpha values that will be tried
...
for i, part in enumerate(parts):
    ...
    try:
        ...
        initialForecast = croston_output.Forecast[-2:].tolist()[1]
        ...
        for dateMonday in rrule.rrule(rrule.WEEKLY, dtstart=start_date, until=end_date): #end_date
            ...
            if dateMonday.isocalendar().week == 52: # in the last week, we put NaN values for forecast and the forecast accuracy metrics. Because in the inventory system script for this week there will not be calculated a new order-up-to-level and we also don't have the actuals since these are in 2023
                #print('last week')
                df_test.loc[df_test['date'] == dateMonday, 'mean_forecast'] = np.nan
                ...
                lastForecast = df_test.loc[df_test['date'] == (end_date - datetime.timedelta(weeks=forecastHorizon))]['mean_forecast'].squeeze()
                df_test.loc[df_test['date'] == dateMonday, 'mean_forecast'] = lastForecast
                ...
                forecast = croston_output.Forecast[-forecastHorizon:].tolist()
                ...
                # add to df_test the mean_forecast and performance metrics
                df_test.loc[df_test['date'] == dateMonday, 'mean_forecast'] = forecast[0]
            ...
            if dateMonday.isocalendar().week == 1:
                ...
                cumulative_forecast_tMinusn = forecast_forecast_tMinusn * len(actuals_i_tMinusNplusOne)
            ...
            # in case all weeks of actuals are in 2022 and the forecast is also made in 2022
                ...
                forecast_forecast_tMinusn = df_test.loc[df_test['date'] == date_forecast_tMinusn]['mean_forecast'].squeeze()
                cumulative_forecast_tMinusn = forecast_forecast_tMinusn * forecastHorizon
        ...
        list_forecastedDemand = [initialForecast] # initiate list with forecast for first week
        list_forecastedDemand.extend(df_test['mean_forecast'].tolist()) # append the forecast column to it
        ...
        num_nan = df_test['periodMSE_horizonAhead'].isna().sum()
        list_forecastedDemand = list_forecastedDemand[:-num_nan] # remove these forecasts from list

        list_actualDemand = df_test['demand'].tolist() # put demand data in list
        list_actualDemand = list_actualDemand[:(-num_nan+1)] # remove last rows, but one less as forecast. Because the last forecast is for the period after. and the first demand period is from the initial forecast
        ...
        dict_forecast_Croston[part] = df_test
...
#######################################################################################
#                          5. Save results Croston's method                           #
#######################################################################################
...
df_output = pd.DataFrame(output, columns=['part', 'best_alpha', 'best_error_crossVal_MSE',  'total_periodMSE_horizonAhead','periodMSE_oneAhead',
                                          'total_periodMASE_horizonAhead','nan_count_MASE_horizonAhead','periodMASE_oneAhead'])
# Save results
df_output.to_csv('Output//model1_Croston///forecast_stats_Croston.csv', index=False)


# Convert list_initialForecast to dataframe
df_initialForecast = pd.DataFrame(list_initialForecast, columns=['part', 'mean_forecast','best_error_crossVal_MSE'])

# Save results
df_initialForecast.to_csv('Output//model1_Croston//df_initialForecast_Croston.csv', index=False)


# Save dictionary with predictions as pickle file
with open('Output//model1_Croston//dict_forecast_Croston.pkl', 'wb') as f:
    pickle.dump(dict_forecast_Croston, f)
